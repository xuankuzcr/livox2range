...

{
    ...
    dynamic_reconfigure::Server<livox2range::RangeImageConfig> server;
    dynamic_reconfigure::Server<livox2range::RangeImageConfig>::CallbackType callback;
    ...
    pub_Img_=nh.advertise<sensor_msgs::Image>("/camera/aligned_depth_to_color/image_raw", 10);  //发布到/image话题
    ros::spin();
}
...
void Pcl2ImgCore::point_cb(const sensor_msgs::PointCloud2ConstPtr & in_cloud_ptr)
{
    ...
    pcl::fromROSMsg(*in_cloud_ptr, *current_pc_ptr);  //ros数据类型转为pcl中的数据类型，下面就使用current_pc_ptr了
     ...
     //float fx=1738.290240007892,fy=1723.368300855836,cx=786.9893767591706,cy=322.1041435965373;  //raw

     Eigen::Affine3f sensorPose = (Eigen::Affine3f)Eigen::Translation3f(-0.082984,-0.0221759,0.0727962);
     //Eigen::Affine3f sensorPose = (Eigen::Affine3f)Eigen::Translation3f(-0.0221759,0.0727962,0.082984);//设置相机位姿
     //Eigen::Affine3f sensorPose = (Eigen::Affine3f)Eigen::Translation3f(-26.7593759,-152.184154,-0.082984);//设置相机位姿
...
void Pcl2ImgCore::dynamic_callback(livox2range::RangeImageConfig &config, uint32_t level)
{
